    ...
    if(isset($_GET['add'])){
        ...
        while($row = fetch_array($query)){
            ...
            if($row['product_quantity'] != $_SESSION['product_' . $_GET['add']]){
                ...
                set_massage("We have only " . $row['product_quantity'] ." " ."{$row['product_title']}". " available");
                redirect("../public/checkout.php");
            }
  ...
  foreach ($_SESSION as $name => $value){
      ...
      if($value > 0 ) {
        ...
        while ($row = fetch_array($query)) {
            ...
            <input type="hidden" name="amount_{$amount}" value="{$row['product_preis']}">
    ...
    foreach ($_SESSION as $name => $value) {
        ...
        if ($value > 0) {
            ...
            if (substr($name, 0, 8) == "product_") {
                ...
                VALUES('{$amount}', '{$transcation}', '{$currency}', '{$status}')");
                ...
                while ($row = fetch_array($query)) {
                    $product_price = $row['product_preis'];
                    ...
                    $insert_report = query("INSERT INTO reports(product_id, order_id, product_title, product_preis, product_quantity)
